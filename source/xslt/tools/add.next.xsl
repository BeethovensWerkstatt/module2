...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="mei:note[not(@grace) and not(@cue) and not(@next)][not(ancestor::mei:chord[@grace])]" mode="add.next">
        ...
        <!-- get current tstamp -->
        <xsl:variable name="current.tstamp" select="ancestor-or-self::mei:*[@tstamp][1]/number(@tstamp)" as="xs:double"/>
        ...
        <xsl:variable name="direct.followers.on.staff" select="if($later.events.on.staff) then($later.events.on.staff[number(@tstamp) = $follow-up.tstamp.on.staff and not(parent::mei:chord/@tstamp)]) else()" as="node()*"/>
        <xsl:variable name="direct.followers.on.layer" select="if($later.events.on.layer) then($later.events.on.layer[number(@tstamp) = $follow-up.tstamp.on.layer and not(parent::mei:chord/@tstamp)]) else()" as="node()*"/>
            ...
            <xsl:when test="not(exists($direct.followers.on.layer)) 
                ...
                <xsl:choose>
                    <xsl:when test="$next.staff">
                        ...
                        <!-- get the next events on staff / layer -->
                        <xsl:variable name="measure.followers.on.staff" select="$next.staff//mei:*[@tstamp = '1' and not(local-name() = ('rest','mRest','mSpace')) and not(local-name() = 'note' and parent::mei:chord)]" as="node()*"/>
                        <xsl:variable name="measure.followers.on.layer" select="$next.layer//mei:*[@tstamp = '1' and not(local-name() = ('rest','mRest','mSpace')) and not(local-name() = 'note' and parent::mei:chord)]" as="node()*"/>
