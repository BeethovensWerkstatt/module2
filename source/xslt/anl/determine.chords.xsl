...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="mei:note[ancestor::mei:staff[@trans.semi and @trans.diat]]" mode="resolve.transposing.instruments">
        <xsl:copy>
            ...
            <xsl:variable name="trans.diat" select="xs:integer(ancestor::mei:staff/@trans.diat)" as="xs:integer"/>
            <xsl:variable name="trans.semi" select="xs:integer(ancestor::mei:staff/@trans.semi)" as="xs:integer"/>
            
            <xsl:variable name="diat.pitches" select="('c','d','e','f','g','a','b')" as="xs:string+"/>
            <xsl:variable name="diat.index" select="index-of($diat.pitches,$this/@pname)" as="xs:integer"/>
            <xsl:variable name="new.diat" select="(7 + $diat.index + $trans.diat) mod 7" as="xs:integer"/>
            <xsl:variable name="new.pname" select="$diat.pitches[if($new.diat = 0) then(7) else($new.diat)]" as="xs:string"/>
            ...
            <xsl:variable name="accid.offset" as="xs:integer">
                <xsl:choose>
                    <xsl:when test="$this/@accid='s'"><xsl:value-of select="1"/></xsl:when>
                    <xsl:when test="$this/@accid.ges='s'"><xsl:value-of select="1"/></xsl:when>
                    <xsl:when test="$this/@accid='f'"><xsl:value-of select="-1"/></xsl:when>
                    <xsl:when test="$this/@accid.ges='f'"><xsl:value-of select="-1"/></xsl:when>
                    <xsl:otherwise><xsl:value-of select="0"/></xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <xsl:variable name="new.semi.base.value" select="$semi.pitches[if($new.diat = 0) then(7) else($new.diat)]" as="xs:integer"/>
            <xsl:variable name="new.semi.real.value" select="(12 + $semi.base.value + $accid.offset + $trans.semi) mod 12" as="xs:integer"/>
            <xsl:variable name="semi.dist" select="$new.semi.real.value - $new.semi.base.value" as="xs:integer"/>
            <xsl:variable name="new.accid" as="xs:string?">
                <xsl:choose>
                    <xsl:when test="$semi.dist = -1"><xsl:value-of select="'f'"/></xsl:when>
                    <xsl:when test="$semi.dist = 1"><xsl:value-of select="'s'"/></xsl:when>
                </xsl:choose>
            </xsl:variable>            
