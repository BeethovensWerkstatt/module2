<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mei="http://www.music-encoding.org/ns/mei" xmlns:custom="none" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:uuid="http://www.uuid.org" exclude-result-prefixes="xs math xd mei custom uuid" version="3.0">
    ...
    <xsl:template match="mei:mdiv">
        <xsl:choose>
            <xsl:when test="exists(mei:score) and not(mei:parts)">
                <xsl:variable name="score" as="node()" select=".//mei:score">
                    <!--<xsl:for-each select=".//mei:score">
                        <xsl:apply-templates select="." mode="split.score.prep"/>
                    </xsl:for-each>-->
                </xsl:variable>
                <!--<xsl:comment>splitted score</xsl:comment>-->
                <xsl:copy>
                    <xsl:apply-templates select="@*" mode="#current"/>
                    <parts xmlns="http://www.music-encoding.org/ns/mei">
                        <xsl:for-each select="$score//mei:staffDef">
                         <xsl:element name="part">
                             <xsl:variable name="this.n" select="$score//mei:staffDef/@n" as="xs:integer+"/>
                             <xsl:attribute name="n" select="$this.n"/>
                             <xsl:for-each select="$score//mei:scoreDef">
                                 <xsl:copy>
                                     <xsl:apply-templates select="$score//mei:scoreDef/@*"/>
                                     <xsl:apply-templates select="$score//mei:staffDef"/>
                                 </xsl:copy>
                             </xsl:for-each>
