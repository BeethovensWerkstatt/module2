<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mei="http://www.music-encoding.org/ns/mei" xmlns:custom="none" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:uuid="http://www.uuid.org" exclude-result-prefixes="xs math xd mei custom uuid" version="3.0">
    ...
    <!-- mode add.invariance -->
    <xsl:template match="mei:harm[@type = 'eventDensity']" mode="compare.event.density">
        ...
        <xsl:variable name="match" as="node()?">
            <xsl:choose>
                <xsl:when test="$is.file.1">
                    <xsl:sequence select="parent::mei:*/mei:harm[@type = 'eventDensity' and @tstamp = $tstamp and number(@staff) gt $first.file.staff.count]"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:sequence select="parent::mei:*/mei:harm[@type = 'eventDensity' and @tstamp = $tstamp and number(@staff) le $first.file.staff.count]"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:choose>
            ...
            <xsl:when test="$is.file.1 and not($match)">
                <xsl:copy>
                    ...
                    <xsl:attribute name="type" select="'eventDensity file1only'"/>
            ...
            <xsl:when test="not($is.file.1) and not($match)">
                <xsl:copy>
                    ...
                    <xsl:attribute name="staff" select="($first.file.staff.count + 1)"/>
                    <xsl:attribute name="place" select="'above'"/>
                    <xsl:attribute name="type" select="'eventDensity file2only'"/>
                    <xsl:apply-templates select="node()" mode="#current"/>
                </xsl:copy>
            </xsl:when>
        </xsl:choose>
